...
const Income = (props: any) => {
  ...
  const renderItem = (item: any, index: number) => {
    ...
    return (
      <div style={{ display: '-webkit-box' }} className="flex text-xs mt-2" key={index}>
        <p className="w-45 border-l-0 border-r-0 px-2 py-4 border-width-[0.5px] border-color-[#e9e9e9] border-solid">
          {income}
        </p>
        <p className="order-table py-4">{order_num}</p>
        <p className="order-table py-4">{staff_cost}</p>
        <p className="order-table py-4">{earn}</p>
        <p className="order-table py-4">{parent}%</p>
        <p className="order-table py-4 text-right">{total}</p>
      </div>
    )
  }
  ...
  const orderList = () => {
    return (
      <div ref={ref} className="flex flex-col pt-5 overflow-x-auto w-700px">
  ...
  return (
    <div className="flex w-[calc(100vw-60px)] min-h-[calc(100vh-60px)]">
      ...
      <div className={`flex flex-col flex-1  items-center pt-20`}>
        <div className="flex w-full justify-between items-center w-700px">
          <p className="font-600 text-2xl">{userInfo?.user_type === 1 ? '渠道列表' : '员工列表'}</p>
          <Button
            onClick={orderDown}
        ...
        <div className="w-700px bg-[#f6f6f6] flex justify-end py-5 rounded-md my-4">
