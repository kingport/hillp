...
import PcDuration from './PcDuration'
...
const Duration = (props: any) => {
  ...
  const getOrderList = async () => {
    return await DataBaseApi.getDurationDetail({
  ...
  const renderItem = (item: any, index: number) => {
    ...
    return (
      <div style={{ display: '-webkit-box' }} className="flex text-xs mt-2" key={index}>
        <p className="w-[25%] rounded-l-lg border-r-0 px-2 py-2 border-width-[0.5px] border-color-[#e9e9e9] border-solid">
          {income}
        </p>
        <p className="order-table w-[25%]">{order_num}</p>
        <p className="order-table w-[25%]">{parent}%</p>
        <p className="order-table w-[25%] text-right">{amount}</p>
      </div>
    )
  }
  ...
  return (
    <>
      <div className="flex flex-col items-center pt-[48px] pb-[90px] w-full sm:(hidden)">
        <HeaderNav renderRight={<NavRightList />} title="时长" />
