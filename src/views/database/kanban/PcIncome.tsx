  ...
  const renderItem = (item: any, index: number) => {
    ...
    return (
      <div style={{ display: '-webkit-box' }} className="flex text-xs mt-2" key={index}>
        <p className="w-[20%] border-r-0 border-l-0 px-2 py-4 border-width-[0.5px] border-color-[#e9e9e9] border-solid">
          {income}
        </p>
        <p className="order-table w-[20%] py-4">{deposit}</p>
        <p className="order-table w-[20%] py-4">{amount}</p>
        <p className="order-table w-[20%] py-4">{parent}%</p>
        <p className="order-table w-[20%] py-4 text-right">{total}</p>
      </div>
    )
  }
    ...
    <div className="flex w-[calc(100vw-60px)] min-h-[calc(100vh-60px)]">
      ...
      <div className={`flex flex-col flex-1  items-center pt-20`}>
        <div className="flex w-full justify-between items-center w-600px">
          <p className="font-600 text-2xl">收款方式</p>
          <Button
            onClick={orderDown}
